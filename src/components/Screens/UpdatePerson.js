...
export class UpdatePerson extends React.Component {
  constructor(props) {
        debugger;
        super(props);
        this.pressed = false;
  ...
  console.log(this.props.edit_person,'this.props.edit_person.data');
  ...
  this.setState({languages:ddl});
    ...
    for(let i=0 ; i<length ; i++){
      ...
      if (db_ccategory.indexOf(t) >= 0) {
        ...
        ss.push(ts);var result = ss.reduce((unique, o) => {
      ...
      ddds.push(ts);  var result1 = ddds.reduce((unique, o) => {
            if(!unique.some(obj => obj.value === o.value )) {
              unique.push(o);
            }
            return unique;
...
photo_n.push (tt1);
  ...
  console.log(img_n);
    ...
    pcategory.push(this.state.person_case_category[0]);
      if(this.state.photo==null){
        ...
        loaderHandler.hideLoader();
      } else if(this.state.person_case_type.length==0){
        this.pressed = false;
        AlertBox('Please Select Atleast One Person Type');
        loaderHandler.hideLoader();
        ...
        var params ={
          ...
          person_id:this.props.edit_person.data[0].person_id,
          update_new:1
        }
        console.log(params,'params');

        this.props.dispatch(faceActions.setEmpty({}));
        this.props.dispatch(faceActions.updatePerson({params:params}));
      }
  ...
  onPressSaveDetect(nextProps){
    var ss =[];
    j=0;
    this.setState({dd_length:nextProps.updatePersonResponse.pgi.length});
    for(let i=0; i<this.state.person_case_type.length;i++){
      var t = this.state.person_case_type[i].name;
      ss.push(t);
    }
    console.log(ss,'ss');
    for(let i=0;i<nextProps.updatePersonResponse.pgi.length ;i++){
      var img_name=nextProps.updatePersonResponse.pgi[i];
      if(this.state.photo.fileSize){
        var dd1 = this.state.photo.uri.split('/');
        var ss1 = dd1.slice(-1).pop();
      ...
      var detect_data = {
        ...
        var params={
          name:nextProps.updatePersonResponse.data.name,
          psqno:nextProps.updatePersonResponse.data.id,
          folderNames:ss.join(','),
          imageName:img_name,
          image:detect_data
        };
  ...
  componentWillUpdate(nextProps) {
    ...
    if(nextProps.isUpdatedPerson == true){
      console.log(nextProps.updatePersonResponse,'isUpdatedPerson');
  ...
  let CheckIndex = i => {
    ...
    return (
      ...
      <View  style={{paddingLeft:10,paddingRight:10}}>
        <Form>
        {/*<Label style={{fontSize:12,fontFamily:'Roboto-Black',paddingLeft:15}}>Saved Images</Label>*/}
        {this.props.edit_person.data.length<1?null:<ScrollView horizontal={true} style={{paddingLeft:10}}>
            {this.props.edit_person.data?this.props.edit_person.data.map((i,index) => {
              return (<TouchableOpacity style={{flexDirection:'row'}} key = {index} ><Image style={{width: 50, height: 50, resizeMode: 'contain'}}  source={{uri: SERVICE_URL+`enrolled_images/${i.folder}/${i.photo}`}} /></TouchableOpacity>)
            }) : null}

          </ScrollView>}
          {/*<Label style={{fontSize:12,fontFamily:'Roboto-Black',paddingLeft:15}}>Upload Images</Label>*/}
        {(this.state.photo!='')?<ScrollView horizontal={true} style={{marginTop:10,paddingLeft:10,paddingRight:10}}>
...
const mapStateToProps = (state) => {
  ...
  return {
    ...
    isUpdatedPerson:faceSelectors.isUpdatedPerson(state),
    updatePersonResponse:faceSelectors.getUpdatePersonResponce(state),
    ...
    edit_person:faceSelectors.getUpdateData(state),

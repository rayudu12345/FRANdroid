...
this.setState({ table_access});
  ...
  _submitCircle(sub_division_id){
    var params={
      sub_division_id:sub_division_id
    };
    this.props.dispatch(dropdownActions.subDivisionIdBasedCircle({params:params}));
  }
  ...
  _getSubDivision(){
    var divison_picker = [];
       divison_picker.push(<Picker.Item key='' label ='Please Select Sub Divison' value =''/>);
        if(this.props.divisions.data!=undefined){
        for(let i=0 ; i<this.props.divisions.data.length ; i++){

          t = this.props.divisions.data[i].name;
          s = this.props.divisions.data[i].id;
          divison_picker.push(<Picker.Item key={i} label ={t} value ={s}/>);
      }
    }

      return(
        ( divison_picker.length>0)?
        <Picker style={{marginLeft:5,borderWidth:1,borderColor:'#ccc'}}
          selectedValue={this.state.sub_division_id}
          onValueChange={(sub_division_id) => ( this.setState({sub_division_id:sub_division_id},this._submitCircle(sub_division_id)))} >
  ...
  _getCircle(){
    var circle_picker = [];
       ...
       circle_picker.push(<Picker.Item key='' label ='Please Select Police Station By Circle Inspector' value =''/>);
        if(this.props.circle.data!=undefined){
        for(let i=0 ; i<this.props.circle.data.length ; i++){

          t = this.props.circle.data[i].name;
          s = this.props.circle.data[i].id;
          circle_picker.push(<Picker.Item key={i} label ={t} value ={s}/>);
      }
    }

      return(
        ( circle_picker.length>0)?
        <Picker style={{marginLeft:5,borderWidth:1,borderColor:'#ccc'}}
          selectedValue={this.state.circle_id}
          onValueChange={(circle_id) => ( this.setState({circle_id:circle_id}))} >

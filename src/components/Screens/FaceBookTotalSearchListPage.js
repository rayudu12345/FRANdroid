...
export class FaceBookTotalSearchListPage extends Component {
  constructor(props) {
    debugger;
    ...
    const { page } = this.state;
      ...
      this.setState({ loading: true });
          fetch(SERVICE_URL+`facebook-not-matched-records-list?&page=${page}`)
        ...
        return(
          ...
          <View style={{height:40,marginLeft:15,marginRight:15,borderBottomColor:'#ccc',borderBottomWidth:0.5,flexDirection:'row',backgroundColor:'#fff'}}>
            <View style={{flex:0.6,justifyContent:'center'}}>
              <Text style={{color:'#374176',fontSize:height/40}}>Facebook Search Detected List</Text>
            </View>
          ...
          <View style={{borderBottomWidth:0.5,marginLeft:15,marginRight:15,marginTop:10,paddingBottom:10,borderBottomColor:"#ccc",flexDirection:'row'}}>
            <View  style={{flex:0.2}}>
              <Text style={{paddingRight:8,fontSize:10,fontWeight:'600',color:'#000'}}>Subject Image</Text>
            </View>
            <View style={{flex:0.4}}>
              <Text style={{fontSize:10,fontWeight:'600',color:'#000'}}>Searched on</Text>
            </View>
            <View style={{flex:0.4}}>
              <Text style={{fontSize:10,fontWeight:'600',color:'#000'}}>Detected by</Text>
            </View>
          ...
          renderItem={({ item }) =>(<TouchableOpacity  style={{borderBottomColor:'#ccc',borderBottomWidth:0.5}}>
                    <View style={{paddingTop:8,paddingBottom:8,flexDirection:'row'}}>
                      ...
                      source={{uri:SERVICE_URL+`fr_images/facebook-search-history/${item.source_image}`}}
                      style={{height:50,width:50,borderRadius:25}}/>
                      </View>
                      <View style={{flex:0.4,justifyContent:'center'}}>
                        <Text style={{color:'#000'}} numberOfLines={1}>{item.searched_at}</Text>
                      </View>
                      <View style={{flex:0.4,justifyContent:'center'}}>
                        <Text style={{color:'#000'}} numberOfLines={1}>{item.detected_by}</Text>
                      </View>
                    </View>
                  </TouchableOpacity>
                )}
                keyExtractor={item => item.searched_id}
...
export default connect(mapStateToProps)(FaceBookTotalSearchListPage);
